...
class ShapExplainer:
    ...
    def __init__(self, model, background_data: torch.Tensor, feature_names: Optional[List[str]] = None):
        ...
        try:
            # Use the device of the first parameter if available
        ...
        class LogModelWrapper(torch.nn.Module):
            def __init__(self, model):
                super().__init__()
                self.model = model
            def forward(self, x):
                return torch.log(self.model(x) + 1e-9)

        self.wrapped_model = LogModelWrapper(self.model)
        self.explainer = shap.GradientExplainer(self.wrapped_model, self.background_data)
